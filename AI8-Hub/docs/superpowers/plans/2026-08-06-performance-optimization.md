...
- Modify: `.env.example`
- Test: `tests/config-blacklist.test.js` pattern — add a small assertion file OR extend runtime config test if exists.
...
**Files:**
- Modify: `docs/superpowers/specs/2026-08-06-performance-optimization-design.md` (mark complete) — optional
- Test: full suite
...
Run: `Invoke-WebRequest http://localhost:7865/v1/models` with `Accept-Encoding: gzip` — expect `Content-Encoding: gzip`. Confirm streaming chat response has no `content-encoding`.
...
**Spec coverage:**
- Spec §1 (connection pool) → Task 1 ✓
- Spec §2 (compression) → Task 2 ✓
- Spec §3 (cache prewarm) → Task 5 ✓
- Spec §4 (SSE parser) → Tasks 6, 7 ✓
- Spec §5 (NODELAY) → Task 8 ✓
- Spec §6 (log throttling) → Tasks 3, 4 ✓
- Global constraint "all tests green" → Task 2/4/6/7/9 checkpoints ✓

**Placeholder scan:** No TBD/TODO; every step has concrete code or commands. Step 6.1's NOTE explains the testing limitation and the resolution path. ✓

**Type consistency:** `initHttpPool` returns Agent and is exported once; `logSlowThresholdMs` spelled identically in Task 3 (config), Task 4 (`getConfig().logSlowThresholdMs`), and tests; `readOffset` used consistently within each parser task. ✓
